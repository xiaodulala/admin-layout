...
var (
	...
	output     = pflag.String("output", "", "output file name; default srcdir/<type>_string.go")
	trimprefix = pflag.String("trimprefix", "", "trim the `prefix` from the generated constant names")
	buildTags  = pflag.String("tags", "", "comma-separated list of build tags to apply")
	doc        = pflag.Bool("docs", false, "if true only generate error code documentation in markdown format")
)
	...
	if outputName == "" {
		...
		if len(pflag.Args()) == 1 {
			baseName = fmt.Sprintf(
				"%s_generated.go",
				strings.ReplaceAll(filepath.Base(strings.TrimSuffix(pflag.Args()[0], ".go")), "-", "_"),
			)
		}
...
func (g *Generator) generate(typeName string) {
	...
	g.Printf(`import "github.com/xiaodulala/admin-layout/pkg/errors"`)
	g.Printf("\t// init register error codes defines in this source code to `github.com/xiaodulala/admin-layout/pkg/errors`\n")
	g.Printf("func init() {\n")
	for _, v := range values {
		code, description := v.ParseComment()
		g.Printf("\terrors.RegisterErrCoder(%s, %s, \"%s\")\n", v.originalName, code, description)
	}
	g.Printf("}\n")
}
...
func (g *Generator) generateDocs(typeName string) {
	...
	tmpl, _ := template.New("docs").Parse(errCodeDocPrefix)
